...
function getRecentEmail(auth) {
    ...
    getEmailById(auth, message_id)
    listenToMailbox(auth, 'me')
    });
}
...
function getEmailById(auth, mail_id){
    gmail.users.messages.get({auth: auth, userId: 'me', 'id': mail_id}, function(err, response) {
        if (err) {
            console.log('The API returned an error: ' + err);
            return;
        }
